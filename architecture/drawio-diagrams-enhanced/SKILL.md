...
## Avoiding Line and Text Overlap

Two distinct problems cause text to render on top of lines or other shapes. Each has a specific fix.

### Problem 0: Lines route through blocks

draw.io's orthogonal auto-router has no awareness of other shapes — it only avoids the source and target. When a source and target are in different rows AND different columns (diagonal relationship), the auto-routed path will travel through whatever blocks happen to be in between.

**Fix: always add explicit `exitX/Y`, `entryX/Y` port constraints and manual `<Array as="points">` waypoints on every edge that connects shapes that are not directly adjacent.**

The approach:
1. Map all block bounding boxes on paper first: (x_left, x_right, y_top, y_bottom) for every shape.
2. Identify the clear corridors — gaps between columns and between rows.
3. Route each edge through those corridors using waypoints, never letting any segment's X or Y range overlap a block's bounding box.
4. **Offset parallel edges** — if two edges share the same corridor, give them different X or Y values (e.g. x=775 and x=790) so they don't draw on top of each other.

**Example — a 3-column, 2-row ERD layout:**

```
col1 (x=100–360)   col2 (x=470–730)   col3 (x=820–1080)
  [Job]              [Employee]          [WorkRelation]     ← row1 y=70–290
  [Contract]         [Department]        [Leave]            ← row2 y=380–635

Column gaps:  col1–col2: x=360–470, use x=415
              col2–col3: x=730–820, use x=775 (and x=790 for a second edge)
Row gap:      row1–row2: y=290–380, use y=335
```

```xml
<!-- Job → Employee: both in row1, direct horizontal in col1-col2 gap -->
<mxCell id="72" ... exitX=1;exitY=0.5;...entryX=0;entryY=0.5;... edge="1" source="Job" target="Employee">
  <mxGeometry relative="1" as="geometry">
    <Array as="points">
      <mxPoint x="415" y="125"/>  <!-- exit Job right, hold at gap x=415 -->
      <mxPoint x="415" y="180"/>  <!-- drop to Employee mid-y, then enter -->
    </Array>
  </mxGeometry>
</mxCell>

<!-- Employee → WorkRelation: col2 right → col2-col3 gap x=775 → drop to WorkRelation mid -->
<mxCell id="70" ... exitX=1;exitY=0.5;...entryX=0;entryY=0.5;... edge="1" source="Employee" target="WorkRelation">
  <mxGeometry relative="1" as="geometry">
    <Array as="points">
      <mxPoint x="775" y="180"/>  <!-- Employee mid-y=180, run right to gap -->
      <mxPoint x="775" y="290"/>  <!-- drop to WorkRelation mid-y=290 -->
    </Array>
  </mxGeometry>
</mxCell>

<!-- Employee → Leave: same corridor but offset to x=790 to avoid overlap with id=70 -->
<mxCell id="74" ... exitX=1;exitY=0.5;...entryX=0;entryY=0.5;... edge="1" source="Employee" target="Leave">
  <mxGeometry relative="1" as="geometry">
    <Array as="points">
      <mxPoint x="790" y="180"/>  <!-- offset from id=70's x=775 -->
      <mxPoint x="790" y="557"/>  <!-- drop past all row2 blocks (max y=635) then enter Leave -->
    </Array>
  </mxGeometry>
</mxCell>

<!-- Employee → Contract: diagonal cross — exit bottom, use row gap y=335, go left to Contract centre -->
<mxCell id="73" ... exitX=0.5;exitY=1;...entryX=0.5;entryY=0;... edge="1" source="Employee" target="Contract">
  <mxGeometry relative="1" as="geometry">
    <Array as="points">
      <mxPoint x="600" y="335"/>  <!-- drop to row gap below Employee -->
      <mxPoint x="230" y="335"/>  <!-- go left to Contract centre x=230 -->
    </Array>
  </mxGeometry>
</mxCell>
```

**Key rules for manual waypoints:**
- Every segment must stay within a known clear corridor — verify against all block bounding boxes before writing coordinates.
- Two edges sharing the same corridor must be offset by at least 10px.
- Use `exitX/exitY` and `entryX/entryY` to anchor where the line leaves/enters each shape — without these draw.io re-routes automatically and ignores your waypoints.

### Problem 1: Edge labels render over nodes

draw.io places edge label text directly on the canvas with no background by default. When orthogonal routing takes a line near or through another shape, the label sits on top of it without masking anything behind it.

**Fix: always add `labelBackgroundColor` and `labelBorderColor` to every edge that carries a visible `value`.**

```xml
<!-- Without fix — label can bleed over shapes -->
<mxCell id="70" value="1 : N" style="edgeStyle=orthogonalEdgeStyle;...;endArrow=ERmany;" edge="1" .../>

<!-- With fix — white opaque backing punches out the line and shapes behind -->
<mxCell id="70" value="1 : N" style="edgeStyle=orthogonalEdgeStyle;...;endArrow=ERmany;labelBackgroundColor=#ffffff;labelBorderColor=none;" edge="1" .../>
```

Apply to **all** labelled edges regardless of diagram type: ERDs, flowcharts, PERT, etc.

### Problem 2: Separate text cells overlaid on filled shapes

A common pattern is to create a filled rectangle for a "band" or panel, then place separate `text` vertex cells at absolute Y coordinates inside it — for subtitle lines, keyword lists, audience labels, etc. draw.io renders the shape's own `value` label on top of everything, so those overlay cells fight each other and produce overlapping text.

**Fix: consolidate all content into the shape's own `value` as inline HTML. Remove the separate overlay cells.**

```xml
<!-- BAD: shape + 3 separate floating text cells at absolute Y positions inside it -->
<mxCell id="10" value="Conceptual" style="rounded=1;fillColor=#1a1a1a;fontColor=#ffffff;fontSize=18;" vertex="1" parent="1">
  <mxGeometry x="150" y="80" width="600" height="130" as="geometry"/>
</mxCell>
<mxCell id="11" value="What exists? How does it relate?" style="text;..." vertex="1" parent="1">
  <mxGeometry x="150" y="118" .../>   <!-- overlaps shape's own label -->
</mxCell>
<mxCell id="12" value="Entities · Relationships · Cardinality" style="text;..." vertex="1" parent="1">
  <mxGeometry x="150" y="142" .../>   <!-- overlaps id="11" -->
</mxCell>

<!-- GOOD: everything in one cell's value as HTML -->
<mxCell id="10" value="&lt;b style=&quot;font-size:18px;&quot;&gt;Conceptual&lt;/b&gt;&lt;br&gt;&lt;span style=&quot;font-size:13px;color:#aaaaaa;&quot;&gt;What exists? How does it relate?&lt;/span&gt;&lt;br&gt;&lt;span style=&quot;font-size:12px;color:#E5307D;font-weight:bold;&quot;&gt;Entities · Relationships · Cardinality&lt;/span&gt;" style="rounded=1;fillColor=#1a1a1a;fontColor=#ffffff;fontSize=18;verticalAlign=middle;html=1;whiteSpace=wrap;" vertex="1" parent="1">
  <mxGeometry x="150" y="80" width="460" height="130" as="geometry"/>
</mxCell>
```

Key requirements for inline HTML values:
- The shape style must include `html=1` and `whiteSpace=wrap`
- All HTML must be XML-escaped: `<` → `&lt;`, `>` → `&gt;`, `"` → `&quot;`
- Use `&lt;b&gt;`, `&lt;span style="..."&gt;`, `&lt;br&gt;` for formatting
- Any badge/pill that needs its own fill colour (e.g. a tool tag) should remain a separate sibling cell placed *beside* the band, not overlapping it
...
7. **Edge labels without background**: Always add `labelBackgroundColor=#ffffff;labelBorderColor=none;` to labelled edges
8. **Text cells overlaid on shapes**: Consolidate multi-line content into the shape's own `value` as inline HTML instead of stacking separate `text` vertex cells at absolute coordinates
...
- [ ] All labelled edges have `labelBackgroundColor=#ffffff;labelBorderColor=none;`
- [ ] No separate `text` vertex cells positioned inside filled shapes — use inline HTML `value` instead
